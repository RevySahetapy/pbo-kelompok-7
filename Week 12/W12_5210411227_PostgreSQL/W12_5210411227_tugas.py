...
# CREATE TABLE
# cur.execute("CREATE TABLE public.nasabah (id_nasabah text DEFAULT 3, nama_nasabah text DEFAULT 30, alamat_nasabah text DEFAULT 30)")
# cur.execute("CREATE TABLE public.cabang_bank (kode_cabang text DEFAULT 3, nama_cabang text DEFAULT 30, alamat_cabang text DEFAULT 30)")
# cur.execute("CREATE TABLE public.rekening  (no_rekening text DEFAULT 3, pin integer DEFAULT 11, saldo integer DEFAULT 11, id_nasabah text DEFAULT 3, kode_cabang text DEFAULT 3)")
# cur.execute("CREATE TABLE public.transaksi (no_transaksi text DEFAULT 3, jenis_nasabah text DEFAULT 30, tanggal text DEFAULT 30, jumlah integer DEFAULT 11, id_nasabah text DEFAULT 3)")
# connect.commit()
...
def select_nasabah(no_rekening) :
    try :
        cur.execute("SELECT no_rekening,  nasabah.id_nasabah ,nama_nasabah, saldo, nama_cabang FROM nasabah, rekening, cabang_bank WHERE nasabah.id_nasabah=rekening.id_nasabah AND cabang_bank.kode_cabang=rekening.kode_cabang AND rekening.no_rekening LIKE %s", [no_rekening])#5210411203_FEBRIYAN
            ...
            for x in cur.fetchall() :
                ...
                if no_rekening in x[0] :
                    ...
                    if menu == '1' :
                        ...
                        cur.execute("UPDATE rekening SET saldo = %s WHERE no_rekening LIKE %s", [transaksi, no_rekening])#5210411203_FEBRIYAN
                        connect.commit()
                        cur.execute("INSERT INTO transaksi (no_transaksi, jenis_nasabah, tanggal, jumlah, id_nasabah) VALUES (%s,%s,%s,%s,%s)", [no_transaksi, jenis_nasabah, tanggal, hasil, id_nasabah])
                        connect.commit()
