...
def select() :
    try :
        ...
        cur.execute(sql)#52104111227_REVY SAHETAPY
        print(f"\nData Barang")
        print("==============================================")
        ...
        print("==============================================\n\n")
        kembali()
    except :#52104111227_REVY SAHETAPY
        print("\nEror")
        kembali()
...
def search(kd_brg) :
    try :
        ...
        print(f"\nHasil Pencarian Dari {kd_brg}")#52104111227_REVY SAHETAPY
        print("==============================================")
        for x in cur.fetchall() :
            print(f"Nama Supplier : {x[0]}")
            print(f"Nama Barang   : {x[1]}")
        ...
        print("\nEror / Data Tidak Di Temukan")#52104111227_REVY SAHETAPY
        kembali()
        ...
        try :#52104111227_REVY SAHETAPY
            for x in cur.fetchall() :
                ...
                if kd_brg in x[0] :
                    ...
                    total = qty * harga
                    cur.execute("UPDATE transaksi_brg SET qty = %s, harga = %s WHERE kode_brg LIKE %s", [qty, harga, kode_brg])#52104111227_REVY SAHETAPY
                    connect.commit()
...
def delete(kd_brg):
    cur.execute("SELECT barang.kode_brg, nota.kode_sup ,nama_sup ,nama_brg, tanggal ,tempo, qty, harga, total FROM supplier, barang, transaksi_brg, nota WHERE supplier.kode_sup=nota.kode_sup AND barang.kode_brg=transaksi_brg.kode_brg AND nota.no_nota=transaksi_brg.no_nota AND barang.kode_brg=%s", [kd_brg])
    try :
        for x in cur.fetchall() :#52104111227_REVY SAHETAPY
            kode_brg = x[0]
            kode_sup = x[1]
            if kd_brg in x[0] :
        ...
        kembali()#52104111227_REVY SAHETAPY
...
while True :
    ...
    print("0. Selesai")#52104111227_REVY SAHETAPY
    pilihan = input("Masukan Pilihan : ")

    if pilihan == '1' :
        select()
    elif pilihan == '2' :
        insert()#52104111227_REVY SAHETAPY
    elif pilihan == '3' :
        search(
        ...
        delete(#52104111227_REVY SAHETAPY
            kd_brg  = input("Masukan Kode Barang yg Ingin Di Hapus : ")
        )
    elif pilihan == '0' :
        clear_screen()
        break#52104111227_REVY SAHETAPY