...
def search(kd_mk) :
    try :
        ...
        print(f"\nHasil Pencarian Dari {kd_mk}")#5210411227_REVY SAHETAPY
        print("==============================================")
        for x in cur.fetchall() :
            print(f"Nama Mata Kuliah : {x[0]}")
            print(f"Dosen Pengampu   : {x[5]}")
        ...
        try :#5210411227_REVY SAHETAPY
            for x in cur.fetchall() :
                if kd_mk in x[0] :
                    ...
                    cur.execute("UPDATE kuliah SET waktu = ?, tempat = ? WHERE kode_mk LIKE ?", [waktu, tempat, kd_mk])#5210411227_REVY SAHETAPY
                    connect.commit()
