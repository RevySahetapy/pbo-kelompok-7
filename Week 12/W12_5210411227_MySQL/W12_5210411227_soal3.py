...
def search(kd_brg) :
    try :
        ...
        print(f"\nHasil Pencarian Dari {kd_brg}")#5210411203_FEBRIYAN
        print("==============================================")
        for x in cur.fetchall() :
            print(f"Nama Supplier : {x[0]}")
            print(f"Nama Barang   : {x[1]}")
        ...
        try :#5210411203_FEBRIYAN
            for x in cur.fetchall() :
                ...
                if kd_brg in x[0] :
                    ...
                    total = qty * harga
                    cur.execute("UPDATE transaksi_brg SET qty = %s, harga = %s WHERE kode_brg LIKE %s", [qty, harga, kode_brg])#5210411203_FEBRIYAN
                    connect.commit()
